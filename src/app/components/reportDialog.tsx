    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[425px]">
              ...
              .map((column) => {
                ...
                return (
                  <div key={accessorKey} className="flex items-center space-x-2">
                    <Checkbox
                      id={`report-${String(accessorKey)}`}
                      checked={reportColumns[accessorKey] ?? false}
                      onCheckedChange={(checked) =>
                        setReportColumns(prev => ({ ...prev, [accessorKey]: !!checked }))
                      }
                    />
                    <Label htmlFor={`report-${String(accessorKey)}`} className="capitalize">
        ...
        <DialogFooter>
          ...
          <Button onClick={onGenerateReport} variant="default">GERAR RELATÓRIO</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
